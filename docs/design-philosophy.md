...
Individual Node를 추상적으로 먼저 설계하는 대신, 배서영의 개인 콘텐츠 배포 프로세스를
실사례로 삼아 먼저 검증했다: 흘러나오는 글 → 태깅되어 텍스트 풀에 축적 → 유관한 조각들이
엮여 하나의 개념만 담은 완결된 글(원자성) → 개인 원본 채널에 게시 → 목적·성격에 따라
여러 조직/채널로 파생 게시.

이 흐름은 5장의 "조직이 개인을 초청받는다" 원칙을 콘텐츠 단위에서 그대로 증명한다 —
원본은 항상 개인에게 있고, 바디템플·현현 같은 조직은 파생본을 가져갈 뿐이다.
조각→글, 글→파생본의 계보는 별도 필드가 아니라 6장의 ConnectionNode(`relation: generates`)로
기록한다. 상세 설계와 스키마는 `docs/content-pipeline.md`.
