...
```bash
export KYTOS_DIR=$HOME/kytos-data
```

조직 협업을 한다면 공유 레포도 추가합니다:

```bash
export KYTOS_DIR=$HOME/git/[공유레포이름]
```
...
`KYTOS_DIR`이 설정된 상태에서 `/task-end`를 실행하면 개인 레포와 공유 레포에 동시에 커밋됩니다.
...
1. 해당 파일을 `$KYTOS_DIR/nodes/`로 옮깁니다
...
```bash
cd $KYTOS_DIR && git log --oneline -3
```
...
개인 레포(`KYTOS_DIR`)는 그대로 유지하고, 공유 레포(`KYTOS_DIR`)를 추가로 설정합니다.
...
`~/.zshrc`에 `KYTOS_DIR`을 추가합니다. `KYTOS_DIR`은 그대로 유지합니다.

```bash
export KYTOS_DIR=$HOME/git/[공유레포명]
```
...
```bash
echo $KYTOS_DIR   # → 개인 레포 경로
echo $KYTOS_DIR    # → 공유 레포 경로
```
