...
```bash
export KYTOS_INDIVIDUAL_DIR=$HOME/kytos-data
```
...
1. 해당 파일을 `$KYTOS_INDIVIDUAL_DIR/nodes/`로 옮깁니다
...
```bash
cd $KYTOS_INDIVIDUAL_DIR && git log --oneline -3
```
...
개인 레포(`KYTOS_INDIVIDUAL_DIR`)는 그대로 유지하고, 공유 레포(`KYTOS_ORG_DIR`)를 추가로 설정합니다.
`/task-end` 실행 시 두 레포에 동시에 커밋됩니다.

### 공유 레포 클론

팀 관리자로부터 초대를 수락한 후 클론합니다.

```bash
git clone https://github.com/[조직명]/[공유레포명].git ~/git/[공유레포명]
```

### 환경변수 추가

`~/.zshrc`에 `KYTOS_ORG_DIR`을 추가합니다. `KYTOS_INDIVIDUAL_DIR`은 그대로 유지합니다.

```bash
export KYTOS_ORG_DIR=$HOME/git/[공유레포명]
```
...
```bash
echo $KYTOS_INDIVIDUAL_DIR   # → 개인 레포 경로
echo $KYTOS_ORG_DIR    # → 공유 레포 경로
```
...
이름과 마지막 세션 요약이 뜨면 완료입니다. 이후 `/task-end` 실행 시 개인·공유 레포에 동시 커밋됩니다.

---

## 문제가 생겼을 때

- `KYTOS_INDIVIDUAL_DIR` 없다는 오류 → 4단계 재확인
