...
```bash
cat $KYTOS_DATA_DIR/individual/me.json
```

`id`가 `did:kytos:YOUR_NAME` 그대로면 아직 작성 전 → 인터뷰 진행.
이미 작성되어 있으면:
> `me.json`이 이미 작성되어 있습니다.
> 수정하려면 `$KYTOS_DATA_DIR/individual/me.json`을 직접 편집하거나 계속 진행하려면 enter를 누르세요.
...
확인이 완료되면 Write 도구로 `$KYTOS_DATA_DIR/individual/me.json`을 저장합니다.
